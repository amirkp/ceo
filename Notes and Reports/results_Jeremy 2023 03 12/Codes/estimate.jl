...
@everywhere begin 
    ...
    function loglike(b)
        ...
        for i =1:n_firms   # for each observation
            ...
            elseif smooth_trim ==1
                if like == 0
                    ll[i] = log(pdf(Normal(),30))
                    n_zeros += 1
                else
                    ll[i]=trim(like/(n_sim*h[1]*h[2]*h[3]),0.1, 300) * (log(like/(n_sim*h[1]*h[2]*h[3])))
                end
            end
        end
        ...
        # output is the average of the log-likelihood contribution of the remaining variables. 
