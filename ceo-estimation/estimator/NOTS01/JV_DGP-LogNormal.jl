...
function sim_data_JV_LogNormal(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up, obs_down, d_min_prof_input, sel_mode="median",size_fe_coeff=0.)
    if flag == false

        up_data = Array{Float64, 2}(undef, 2, n_firms)
        up_data[1,:] = rand(Random.seed!(1234+i), LogNormal(Σ_up[1,1], Σ_up[1,2]), n_firms)
        up_data[2,:] = rand(Random.seed!(1234+200i), LogNormal(Σ_up[2,1], Σ_up[2,2]), n_firms)
    ...
    elseif flag==true # take observed data as given (do not generate observed chars)
        ...
        up_data = Array{Float64, 2}(undef, 2, n_firms)
        up_data[1,:] = obs_up
        up_data[2,:] = rand(Random.seed!(1234+i), LogNormal(Σ_up[2,1], Σ_up[2,2]), n_firms)

        
        down_data = Array{Float64, 2}(undef, 3, n_firms)
        down_data[1:2,:] = obs_down
        down_data[3,:] = rand(Random.seed!(1234+600i), LogNormal(Σ_down[2,1], Σ_down[2,2]), n_firms)
    end
    ...
    for j = 1:n_firms
        C[:,j] .-= size_fe_coeff*down_data[1,j]
    end
    ...
    down_valuation = diag(up_data'*β_down*down_match_data) + down_match_data[1,:].* size_fe_coeff
...
function sim_data_JV_up_obs(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up, d_min_prof_input, sel_mode="median", size_fe_coeff=0.)
    ...
    elseif flag==true # take observed data as given (do not generate observed chars)
        ...
        up_data = Array{Float64, 2}(undef, 2, n_firms)
        up_data[1,:] = obs_up
        up_data[2,:] = rand(Random.seed!(1234+300i),LogNormal(Σ_up[2,1], Σ_up[2,2]), n_firms)
