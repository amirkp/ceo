...
@everywhere begin
    ...
    globT=2*3600; locT=1800; data_mode=3;
    # globT=30; locT=10; data_mode=3;

    bbo_population_size =100
    bbo_max_time=globT; bbo_ndim = 8;
    bbo_search_range =vcat(repeat([(-10.0, 10.0)],5),[(.01, 10.)],[(-10., 5.)], [(.01, 5.)])
...
@everywhere begin 
    ...
    function loglike(b)
        ...
        solve_draw =  x->sim_data_JV_up_obs(bup, bdown , 1., 1., n_firms, 3165160+x, true, up_data,down_data[1:2,:],b[6], sel_mode,  b[7], b[8])
...
b= [-0.0583, 5.6603, 0.7898, -3.8481, 3.97079582423067,4.00,2.165, 0.3517]
...
gen = "NORMAL_S200-pos-dprofmed"; iter =  Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]);
