...
quant_bw(price_data, up_data, 0.3 )
...
obj_linear(b) = objective_f(b ,2, price_data, up_data, 0.3, 0.5)
...
for i =1:length(x_grid)
    obj_linear(b) = objective_f(b ,x_grid[i], price_data, up_data, 0.4, 0.8)
    res = Optim.optimize(obj_linear, [1.0,1.0])
    med_vec[i],der_vec[i] = res.minimizer
end

scatter!(x_grid,med_vec, markersize=1.)
