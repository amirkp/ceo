...
@everywhere begin
    ...
    function replicate(rep)
        ...
        for i = 1:n_firms
            ...
            obj_linear(b) = objective_f(b ,up[1,i], pr, up[1,:], 2.95 - 7.8*epsbar_vec[i] + 6.08*epsbar_vec[i]^2 , epsbar_vec[i])
...
scatter(up[1,:], der_vec, markersize=1, xlims=(0,4), xlabel="x", ylabel=ylab, legend=false, ylims=(-2,10), dpi=300)
scatter(epsbar_vec, der_vec, markersize=1, xlims=(0,1), xlabel="x", ylabel=ylab, legend=false, ylims=(-2,10), dpi=300)


scatter(up[1,:], med_vec, markersize=1, xlims=(0,4), xlabel="x", ylabel=ylab, legend=false, ylims=(-2,10), dpi=300)
scatter(epsbar_vec, der_vec, markersize=1, xlims=(0,1), xlabel="x", ylabel=ylab, legend=false, ylims=(-2,10), dpi=300)
...
function krs(x,eps,hx, he)
    ...
    val = 0.0 
    for i = 1:n_firms
        tw += pdf(Normal(), (x-up[1,i])/hx) * pdf(Normal(), (eps-epsbar_vec[i])/he)
        val += der_vec[i]*pdf(Normal(), (x-up[1,i])/hx) * pdf(Normal(), (eps-epsbar_vec[i])/he)
    end
    return val/tw
end
...
contour(up[1,:],epsbar_vec, der_vec, fill=true)

using Plots; pythonplot()

using PythonPlot

contour(up[1,:],epsbar_vec, der_vec, levels=1)
