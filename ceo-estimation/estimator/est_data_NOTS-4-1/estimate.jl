...
# normal dist, no fixed effect on x 
...
@everywhere begin
    ...
    bbo_search_range =vcat(repeat([(-10.0, 10.0)],6), [(.01, 5.)])
...
@everywhere begin 
    ...
    function loglike(b)
        ...
        solve_draw =  x->sim_data_JV_up_obs(bup, bdown , 1., 1., n_firms, 3165160+x, true, up_data,down_data[1:2,:],b[6], sel_mode,  0., b[7])
...
gen = "NORMAL_S200-noxlin"; iter =  Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]);
