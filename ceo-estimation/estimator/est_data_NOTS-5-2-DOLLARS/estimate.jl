...
# CEO COMP is in MILLION DOLLLARS 
# the standalon firm specific shock is log-normal 
# in DOLLAR-1 the firm characteristic eta was log-normal 
# now it is normal
...
@everywhere begin
    ...
    bbo_max_time=globT; bbo_ndim = 11;
    bbo_search_range =vcat(repeat([(-20.0, 20.0)],9),[(0., 10)], [(.01, 10.)])
...
@everywhere begin 
    ...
    function loglike(b)
        ...
        for i =1:n_firms
            ...
            for j =1:n_sim
                ...
                if (
                    pdf(Normal(),((down_data[1,i] - sim_dat[j][2][1,i])/h[1]))
                    *pdf(Normal(),((down_data[2,i] - sim_dat[j][2][2,i])/h[2]))
                    *pdf(Normal(),((price_data[i] - sim_dat[j][3][i])/h[3])))  == 0
                    count_sim_zero+=1 
                end
        ...
        if mod(time(),10)<10
            ...
            println("sim zero is ", count_sim_zero)
        end
        Random.seed!()
        return -out
    end
...
b = [-0.72, 1.086, 0.982, 1.515, -0.719, 1.731, 3.719, 4.839, 1.542, 2.748, 1.8597];
...
gen = "RANDOM-SAMPLE-DOLLAR_PRICE-scaledxi"; iter =  Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]);
