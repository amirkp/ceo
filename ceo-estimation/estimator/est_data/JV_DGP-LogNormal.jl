...
function sim_data_JV_up_obs(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up,obs_down, d_min_prof_input, sel_mode="median", size_fe_coeff=0., scale_unobs=0.1)
    ...
    elseif flag==true # take observed data as given (do not generate observed chars)
        ...
        up_data = Array{Float64, 2}(undef, 2, n_firms)
        up_data[1,:] = obs_up
        up_data[2,:] = rand(Random.seed!(1234+300i),Uniform(0., 1.), n_firms)

        
        down_data = Array{Float64, 2}(undef, 3, n_firms)
        down_data[1:2,:] = obs_down
        down_data[3,:] = rand(Random.seed!(1234+6i), LogNormal(0,scale_unobs), n_firms)
    end
    ...
    for j = 1:n_firms
        C[:,j] .-= size_fe_coeff[2]*down_data[2,j] + size_fe_coeff[1]*down_data[1,j]
    end
    ...
    down_valuation = diag(up_data'*β_down*down_match_data) + down_match_data[2,:].* size_fe_coeff[2] + down_match_data[1,:].* size_fe_coeff[1]
