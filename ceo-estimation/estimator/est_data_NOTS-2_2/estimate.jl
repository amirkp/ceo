...
@everywhere begin
    ...
    bbo_max_time=globT; bbo_ndim = 8;
    bbo_search_range =vcat(repeat([(-10.0, 10.0)],4),[(.000001, 10.)],[(.000001, 10.)],[(-10.0, 10.0)], [(.01, 5.)])
    ...
    # down_data[2,:]= exp.(down_data[2,:])
    # price_data = exp.(price_data)
...
@everywhere begin 
    ...
    function loglike(b)
        ...
        solve_draw =  x->sim_data_JV_up_obs(bup, bdown , 1., 1., n_firms, 3160+x, true, up_data,down_data[1:2,:],b[6], sel_mode,  b[7], b[8])
        sim_dat = pmap(solve_draw, 1:n_sim, batch_size=4)
        ll=zeros(n_firms)
        n_zeros = 0
...
loglike([-0.0322, 0.0109, 0.0325, -19.2931, 1.0617, 1.6925, 5.4973, 0.3268])

b= res[1,:]
loglike(b[1:8])-b[9]


b= [0.168, 0.0231, -0.1539, -9.4094, 1.6192, 1.9912, 6.0953, 0.323]


b =[ 0.420058  0.150374   0.124929   3.95425  10.6238    0.00145844  1.71972  0.0953184]
...
up, down, pr = sim_data_JV_up_obs(bup, bdown , 1., 1., n_firms, 134, true, up_data,down_data[1:2,:],b[6], sel_mode ,b[7],b[8])
...
function pdist(pars)
        ...
        up1, down1, pr1 = sim_data_JV_up_obs(bup, bdown , 1., 1., n_firms, 1234, true, up_data,down_data[1:2,:],b[6], sel_mode ,pars[2],b[8]);
    return sum((pr-pr1).^2)
end
...
scatter(up_data,down_data[2,:], markersize =1 )

cor(up[1,:],down[2,:])
cor(up_data,down_data[2,:])


scatter(up[1,:],down[1,:], markersize =1 )
scatter(up_data,down_data[1,:], markersize =1 )
...
scatter(up_data,price_data, markersize =1 )

cor(up[1,:],pr)
cor(up_data,price_data)


scatter(down[1,:],pr, markersize =1 )
scatter(down_data[1,:],price_data, markersize =1 )
...
scatter(down_data[2,:],price_data, markersize =1 )
...
scatter(down[2,:],pr, markersize =1 )
scatter(up_data,price_data, markersize =1 )
scatter(pr,price_data, markersize =1 )


cor(up_data,down_data[2,:])
cor(up[1,:], down[2,:])

spoints = [b[4], b[7], b[1]] +rand(3).*3
spoints=rand(3)
opt = Optim.optimize(pdist, spoints, show_trace=true);

Optim.minimizer(opt)
opt = Optim.optimize(pdist, Optim.minimizer(opt), show_trace=true);


[b[4], b[7], b[1]] - Optim.minimizer(opt)
...
res_global = bboptimize(opt1, rand(Random.seed!(), 8 ))
...
gen = 2; iter =  Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]);
...
[-0.0322, 0.0109, 0.0325, -9.2931, 1.0617, 1.6925, 5.4973, 0.3268]
