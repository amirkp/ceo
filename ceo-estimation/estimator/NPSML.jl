...
@everywhere function replicate_byseed(n_rep, n_firms, n_sim, h_scale,  par_ind, sel_mode, globT, locT, data_mode)
    ...
    sim_data_JV_LogNormal(bup, bdown, Σ_up, Σ_down, DGPsize
        , 38+n_rep, false, 0, 0, true_pars[6], sel_mode)
    ...
    function loglike(b)
        ...
        solve_draw =  x->sim_data_JV_up_obs(bup, bdown , Σ_up, Σ_down, n_firms, 360+x, true, up_data[1:2,:],b[6], sel_mode)
    ...
    bbo_search_range = [(-10.0, 10.0), (-10.0, 10.0),(-10.0, 10.0),(-10.0, 10.0),(-10.0, 10.0),(-10.0, 10.0)]
...
for n_sim =50:25:50
    for n_firms in [100]
        for data_mode=3:3:3
                est_pars = pmap(x->replicate_byseed(x, n_firms, n_sim,[ 1.,  1., 1.], 1:6, "median", 60, 60, data_mode), 1:n_reps)
