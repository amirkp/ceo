...
df = DataFrame(dProf= up_ders[:,3], X=up_data,  Y1 = y1_[:,2], Y2 = y2_[:,2])
ols = lm(@formula(dProf ~  X+Y2), df)



scatter(up_data, up_ders[:,2], markersize = 2, xlims=(0,5), ylims = (0,20))
scatter!(up_data, up_ders[:,3], markersize = 3, color =:yellow)


scatter(up_data, down_data[1,:], markersize=2)
scatter!(up_data, y1_[:,2], markersize=2)

# scatter(down_data[1,:],price_data, markersize=2)
# scatter!(y1_[:,2],price_data, markersize=2)

scatter(up_data, down_data[2,:], markersize=2)
scatter!(up_data, y2_[:,2], markersize=2)
...
@rput down_data;
@rput price_data;
R"up_ders = compDerEst(up_data, price_data,p=0)";
R"y1_ = looLocPolSmootherC(up_data, down_data[1,], .2, 1, EpaK)";
R"y2_ = looLocPolSmootherC(up_data, down_data[2,], 1., 1, EpaK)";
...
df = DataFrame(dProf= up_ders[:,4], X=up_data,  Y1 = y1_[:,2], Y2 = y2_[:,2])
ols = lm(@formula(dProf ~ X+ Y1+Y2), df)
ols = lm(@formula(dProf ~ X+ Y1+Y2), df)

scatter(up_data, up_ders[:,4], markersize = 2, xlims=(0,5), ylims = (0,20))
...
R"up_der<-predict(up_prof_s, up_data, nderiv=1)"
...
R"d <- data.frame(p = price_data, x = up_data, y1 = down_data[1,], y2= down_data[2,])"
...
df2 = DataFrame(dProf= up_der[:,1], X=up_data,  Y1 = down_match_1[:,1] , Y2 = down_match_2[:,1])
ols = lm(@formula(dProf ~ Y1+Y2+Y1*Y2), df)

ols = lm(@formula(dProf ~ X+X^2+X^3+ Y1 + Y2), df)

pred_df = DataFrame(dProf=2., X=[1.5],  Y1 =[.25] , Y2 = [2])
...
-1.94 * 1.5 + 10.3* 0.25 * 1.5 +0.62*1.5*2
