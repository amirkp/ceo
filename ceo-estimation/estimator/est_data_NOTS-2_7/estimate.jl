...
# Fixed coeff on eps eta. also cpeff on epsy2
...
@everywhere begin
    ...
    bbo_max_time=globT; bbo_ndim = 7;
    bbo_search_range =vcat(repeat([(-10.0, 10.0)],4),[(.000001, 10.)],[(-10.0, 10.0)], [(.01, 5.)])
...
@everywhere begin 
    ...
    function loglike(b)
        ...
        solve_draw =  x->sim_data_JV_up_obs(bup, bdown , 1., 1., n_firms, 3165160+x, true, up_data,down_data[1:2,:],b[5], sel_mode,  b[6], b[7])
...
res_global = bboptimize(opt1, rand(Random.seed!(), bbo_ndim ))
...
gen = "fixed_epseta_epsy2"; iter =  Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]);
