...
function sim_data_JV_up_obs(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up,obs_down, d_min_prof_input, sel_mode="median", size_fe_coeff=[0.1,0.1], scale_unobs=0.1, xcoeff=0.)
    ...
    down_valuation = diag(up_data'*β_down*down_match_data) + down_match_data[2,:].* size_fe_coeff[2] + down_match_data[1,:].* size_fe_coeff[1] +up_data[1,:].*xcoeff
