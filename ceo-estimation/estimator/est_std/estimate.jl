...
@everywhere begin 
    ...
    function loglike(b)
        ...
        sim_dat = pmap(solve_draw, 1:n_sim, batch_size=7)
        ll=zeros(n_firms)
        n_zeros = 0
...
bc = [-0.1593, -0.0009, 0.1592, -4.4274, -1.2922, 4.6879, -0.0545, 1.1679, 0.1936]

lplot = x-> loglike(vcat(x[1],x[2],bc[2:end]))
loglike(bc)
lplot(0)
collect(-2:0.1:1)
lplot.(collect(-2:0.1:1))

scatter(collect(-2:0.1:1),lplot.(collect(-2:0.1:1)) )

Hess = FiniteDiff.finite_difference_hessian(loglike, bc)

jack = jacobian(central_fdm(5, 1), loglike, bc)
dval = central_fdm(5, 1; factor=1e18)(lplot, bc[1])


p1 = nodes(11,:chebyshev_nodes)
p2 = nodes(15,:chebyshev_nodes)

f_approx = chebyshev_interp()
n=10
domain = [-.5,0.5]
...
scatter(up_data,down_data[1,:],
    markersize =2, xlims=(0,5),
    xlabel="x: General Ability Index (GAI)",
    ylabel="y1: Geography and Industry HHI",
    legends = false, smooth=true)



scatter(up_data,down_data[2,:],
markersize =2, xlims=(0,5),ylims=(5,15),
xlabel="x: General Ability Index (GAI)",
ylabel="y2: Log Firm Size",
legends = false, smooth=true)
...
markersize =2, xlims=(0,5),ylims=(5,15),
xlabel="x: General Ability Index (GAI)",
ylabel="Log CEO Compensation",
legends = false, smooth=true)

scatter(down_data[1,:],price_data,
markersize =2, xlims=(0,1.2),ylims=(3,15),
xlabel="y1: Geography and Industry HHI",
ylabel="Log CEO Compensation",
legends = false, smooth=true)
...
markersize =2, xlims=(5,15), ylims=(5,15),
xlabel="y2: Log Firm Size",
ylabel="Log CEO Compensation",
legends = false, smooth=true)